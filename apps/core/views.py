...
def register_view(request):
    if request.method == 'POST':
        ...
        if password1 != password2:
            return render(request, 'admin/register.html', {'error': 'Mật khẩu không khớp!'})

        if User.objects.filter(username=username).exists():
            return render(request, 'admin/register.html', {'error': 'Tên đăng nhập đã tồn tại!'})
...
def manage_tourism(request):
    return render(request, 'admin/tourism.html')